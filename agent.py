...
def cf_run_task(task: str, preflight_context: str = "", rag_context: str = "") -> tuple[str, bool]:
    ...
    system = (
        ...
        "  - CREATE a new file → use write_file (NEVER use run_shell with echo/touch)\n"
        "  - EDIT an existing file → use read_file first, then edit_file\n"
        "  - CHECK system info (disk, RAM, date, processes) → use run_shell\n"
        "  - LIST files → use list_directory\n"
        "  - SEARCH web → use web_search\n"
        "  - GIT staging/commit → use run_shell with git commands\n\n"
        "WORKFLOW for file creation:\n"
        "  1. Call write_file with path and full content\n"
        "  2. Respond with FINAL_ANSWER: <summary>\n\n"
        "WORKFLOW for coding edits:\n"
        "  1. Call read_file to read the target file\n"
        "  2. Call edit_file with old_str copied EXACTLY from the file\n"
        "  3. Respond with FINAL_ANSWER: <summary>\n\n"
